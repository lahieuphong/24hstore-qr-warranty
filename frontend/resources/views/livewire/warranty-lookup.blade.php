    ...
    <div class="w-full space-y-5">
        ...
        @else
            <section class="public-card p-6 text-center sm:p-10">
                <div class="mx-auto grid size-16 place-items-center rounded-full {{ $state === 404 ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-700' }} text-2xl font-black">{{ $state === 404 ? '!' : '↻' }}</div>
                ...
                <div class="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                    ...
                    @if ($state === 503)
                        <button type="button" wire:click="reload" wire:loading.attr="disabled" class="btn-primary">Thử lại</button>
                    @endif
                </div>
            </section>
        @endif
