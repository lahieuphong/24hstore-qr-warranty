...
class Index extends Component
{
    ...
    public function save(AdminActivityLogger $activityLogger): void
    {
        ...
        $activityLogger->record(
            ...
            ($wasEditing ? 'Cập nhật' : 'Thêm').' tài khoản '.$user->email.' với vai trò '.$validated['role'].'.',
            $user,
            ['role' => $validated['role'], 'is_active' => $user->is_active],
        );
    ...
    public function toggleActive(int $id, AdminActivityLogger $activityLogger): void
    {
        $this->authorize('users.manage');
        $user = User::query()->findOrFail($id);
        ...
        $activityLogger->record(
            'user.status_changed',
            ($user->is_active ? 'Mở khóa' : 'Khóa').' tài khoản '.$user->email.'.',
            $user,
